...
def main(data_path, txt_path, model_name="base_plus"):
    ...
    for frame_idx in range(num_frames):
        ...
        if is_empty_or_none(left_mask) or is_empty_or_none(right_mask):
        ...
        if count==0:
            print(f"[WARNING] Frame {frame_idx+1}: Cannot find pairs, skipping.")
            average_dist = -1
        else: 
            average_dist = total_dist*1.0/count
            if average_dist<lower_limit or average_dist>upper_limit:
                print("Horizontal Gauge in", frame_idx+1)
